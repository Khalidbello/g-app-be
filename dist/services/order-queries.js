...
const saveDOrder = (gurasa, suya, name, email, date) => {
    return new Promise((resolve, reject) => {
        const query = 'INSERT INTO defined_orders (suya, gurasa, name, created, email) VALUES (?, ?, ?, ?, ?)';
        connectdb_1.default.query(query, [suya, gurasa, name, date, email], (err, result) => {
            if (err) {
...
const deleteDOrder = (id, email) => {
    return new Promise((resolve, reject) => {
        const query = 'DELETE FROM defined_orders WHERE id = ? AND email = ?';
        connectdb_1.default.query(query, [id, email], (err, result) => {
            if (err) {
                console.log('an error ocured fetching defined order', err);
                reject(false);
            }
